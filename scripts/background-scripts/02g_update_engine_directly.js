// Operations Intelligence — Engine Direct Update
// ===========================================================
// Run in ServiceNow Background Scripts — GLOBAL scope.
// Finds the Scripted REST API operation record and replaces
// the engine script in-place. No manual editing required.
// Run once, then verify with {"op":"selftest"} via the API.
// ===========================================================
(function () {
    var SCOPE    = 'x_infte_ops_int';
    var API_NAME = 'Operations Intelligence Engine';

    // ── 1. Locate the engine operation record ──────────────
    var op = new GlideRecord('sys_ws_operation');
    op.addEncodedQuery(
        'sys_scope.scope=' + SCOPE +
        '^web_service_definition.name=' + API_NAME
    );
    op.setLimit(1);
    op.query();
    ...
    if (!op.next()) {
        gs.print('');
        gs.print('*** ERROR: Scripted REST operation not found.');
        gs.print('*** Scope   : ' + SCOPE);
        gs.print('*** API Name: ' + API_NAME);
        gs.print('*** Open: System Web Services > Scripted REST APIs');
        gs.print('***       then check "Operations Intelligence Engine" exists.');
        return;
    }

    var opSysId = op.getUniqueValue();
    gs.print('Found engine operation: ' + opSysId);

    // ── 2. Replacement engine script ───────────────────────
    var newScript = `
// ============================================================
...
`;
    ...
    // ── 3. Apply the update ────────────────────────────────
    op.setValue('operation_script', newScript);
    op.setWorkflow(false);
    var updated = op.update();

    gs.print('');
    gs.print('================================================================');
    if (updated) {
        gs.print('  Operations Intelligence Engine  —  UPDATED SUCCESSFULLY');
        gs.print('  Operation sys_id : ' + opSysId);
        gs.print('  Script length    : ' + newScript.length + ' chars');
        gs.print('================================================================');
        gs.print('');
        gs.print('  Run this to verify:');
        gs.print('  curl -X POST $INSTANCE$ENGINE_ENDPOINT \\');
        gs.print('    -u $SNOW_USER:$SNOW_PASS \\');
        gs.print('    -H "X-Engine-Key: $ENGINE_KEY" \\');
        gs.print('    -H "Content-Type: application/json" \\');
        gs.print('    -d \'{"op":"selftest"}\'');
        gs.print('');
        gs.print('  Or run: bash test_engine.sh  (from the repo)');
    } else {
        gs.print('  *** UPDATE FAILED — check permissions and retry.');
        gs.print('  *** Ensure you are running in Global scope.');
        gs.print('================================================================');
    }
    gs.print('');
})();
